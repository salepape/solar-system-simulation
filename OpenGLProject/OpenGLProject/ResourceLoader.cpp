...
namespace ResourceLoader
{
	...
	void LoadCelestialBodies()
	{
		...
		/* Ceres */		celestialBodiesRef.emplace_back(BodyData{ texturePaths[9],  0.074f, GetBody("Mars").bodyData.distanceToParent + 2.75f * DIST_SCALE_FACTOR,		0.0f,   1683.15f,  9.07f,   17.14f });
		/* Pluto */		celestialBodiesRef.emplace_back(BodyData{ texturePaths[10], 0.186f, GetBody("Neptune").bodyData.distanceToParent + 39.24f * DIST_SCALE_FACTOR,	57.47f, 90560.0f, -6.39f,   10.62f });

		/* Luna */		celestialBodiesRef.emplace_back(BodyData{ texturePaths[11], 0.273f, GetBody("Earth").bodyData.radius + 0.384f * DIST_SCALE_FACTOR,				6.68f,  27.32f,    27.32f,  5.145f,	   GetBody("Earth").GetID()   });
		/* Callisto */	celestialBodiesRef.emplace_back(BodyData{ texturePaths[12], 0.378f, GetBody("Jupiter").bodyData.radius + 1.883f * DIST_SCALE_FACTOR,			0.0f,   16.69f,    16.69f,  2.017f,	   GetBody("Jupiter").GetID() });
		/* Europa */	celestialBodiesRef.emplace_back(BodyData{ texturePaths[13], 0.245f, GetBody("Jupiter").bodyData.radius + 0.671f * DIST_SCALE_FACTOR,			0.1f,   3.55f,     3.55f,   1.791f,	   GetBody("Jupiter").GetID() });
		/* Ganymede */	celestialBodiesRef.emplace_back(BodyData{ texturePaths[14], 0.413f, GetBody("Jupiter").bodyData.radius + 1.07f * DIST_SCALE_FACTOR,				0.16f,  7.15f,     7.15f,   2.214f,    GetBody("Jupiter").GetID() });
		/* Io */		celestialBodiesRef.emplace_back(BodyData{ texturePaths[15], 0.286f, GetBody("Jupiter").bodyData.radius + 0.422f * DIST_SCALE_FACTOR,			0.0f,   1.77f,     1.77f,   2.213f,	   GetBody("Jupiter").GetID() });
		/* Titan */		celestialBodiesRef.emplace_back(BodyData{ texturePaths[16], 0.404f, GetBody("Saturn").bodyData.radius + 1.222f * DIST_SCALE_FACTOR,				0.0f,   15.95f,    15.95f,  0.0f,      GetBody("Saturn").GetID()  });
		/* Triton */	celestialBodiesRef.emplace_back(BodyData{ texturePaths[17], 0.212f, GetBody("Neptune").bodyData.radius + 0.354f * DIST_SCALE_FACTOR,			0.0f,   5.88f,     5.88f,   129.812f,  GetBody("Neptune").GetID() });
		///* Deimos */	celestialBodiesRef.emplace_back(BodyData{ texturePaths[18], 0.000f, GetBody("Mars").bodyData.radius + 0.156f * DIST_SCALE_FACTOR,				0.0f,   1.263f,    1.263f,	27.58f,	   GetBody("Mars").GetID()	  });
		///* Phobos */	celestialBodiesRef.emplace_back(BodyData{ texturePaths[19], 0.000f, GetBody("Mars").bodyData.radius + 0.94f * DIST_SCALE_FACTOR,				0.0f,	0.319f,    0.319f,  28.4f,	   GetBody("Mars").GetID()	  });
	}
